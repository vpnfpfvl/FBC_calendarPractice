...
class GenerateJuliusCal
  ...
  class << self
    def generate_days(year, month, day)
      ...
      month_first_w.times {days_array.push("\s\s\s")}
      ...
      days_array.each.with_index do |day, index|
        ...
        if @day == "\s\s\s"
          ...
          if year == THIS_Y && month == THIS_M && @day == THIS_D
            optimized_days_array << "\e[47m\e[30m\s\s#{@day}\e[0m\s"
            next
          end
          optimized_days_array << "\s\s#{@day}\s"
        else
          if year == THIS_Y && month == THIS_M && @day == THIS_D
            optimized_days_array << "\e\e[47m\e[30m\s#{@day}\e[0m\s"
            next
          end
...
#{one_month_days_string_result}
  ...
  def generate
    ...
    while all_date_array_for_resource.size / 2 >= 1 do
