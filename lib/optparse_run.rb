...
class OptparseRun
  ...
  def optparse_run
    ...
    argv_parser_on_y(this_argv_parser, argv)
    argv_parser_on_m(this_argv_parser, argv)
    argv_parser_on_3(this_argv_parser, argv)
    argv_parser_on_A(this_argv_parser, argv)
    argv_parser_on_B(this_argv_parser, argv)
    argv_parser_on_j(this_argv_parser, argv)
    argv_parser_on_h(this_argv_parser, argv)

    this_argv_parser.parse(argv)

    argv_parser_on_no_opt(this_argv_parser, argv)
  ...
  private
  def argv_parser_on_y(this_argv_parser, argv)
  ...
  def argv_parser_on_m(this_argv_parser, argv)
  ...
  def argv_parser_on_3(this_argv_parser, argv)
    this_argv_parser.on('-3') do | v |
      ...
      when opt_has_two_numbers(argv, index) && !correct_month(argv, index + 1)
        puts "月は1から12の間で入力してください"
        exit
      when this_argv_has_conflicting_opt(argv, @conflicting_opt_list)
      ...
      when opt_has_two_numbers(argv, index) && correct_month(argv, index + 1) && correct_year(argv, index + 2)
        ...
        @keys_result[:basic_year] = argv[index + 2].to_i
        @keys_result[:pre_month] = 1
        @keys_result[:pre_year] = argv[index + 2].to_i
        @keys_result[:next_month] = 1
        @keys_result[:next_year] = argv[index + 2].to_i
        argv.delete_at(index + 2)
        argv.delete_at(index + 1)
        argv.delete_at(index)
      when opt_has_no_num(argv, index)
  ...
  def argv_parser_on_A(this_argv_parser, argv)
  ...
  def argv_parser_on_B(this_argv_parser, argv)
  ...
  def argv_parser_on_j(this_argv_parser, argv)
  ...
  def argv_parser_on_h(this_argv_parser, argv)
  ...
  def argv_parser_on_no_opt(this_argv_parser, argv)
    case 
