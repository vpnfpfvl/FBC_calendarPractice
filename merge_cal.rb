  ...
  def replace_layout_var(all_dates)
    all_dates.each_with_index do |month, month_index|
      month[2].each_with_index do |element, day|
        ...
        if element.include?('blank_for_1char')
          all_dates[month_index][2][day] = element.gsub(/blank_for_1char/, @layout_status[:blank_for_1char].to_s)
        end
        if element.include?('blank_for_2char')
          all_dates[month_index][2][day] = element.gsub(/blank_for_2char/, @layout_status[:blank_for_2char].to_s)
        end
        if element.include?('blank_for_3char')
          all_dates[month_index][2][day] = element.gsub(/blank_for_3char/, @layout_status[:blank_for_3char].to_s)
        end
      end
    end
  end
